...
OUTER_BOX_DIMS = [
  INNER_BOX_DIMS.x + 2 * BOX_THICKNESS, INNER_BOX_DIMS.y + 2 * BOX_THICKNESS,
  INNER_BOX_DIMS.z + 4 *
  BOX_THICKNESS
];
...
module side(flip = false) {
  ...
  lid_extrusion = [
    OUTER_BOX_DIMS.x - 2 * BOX_THICKNESS, OUTER_BOX_DIMS.y - 2 * BOX_THICKNESS,
    OUTER_BOX_DIMS.z
  ];
  ...
  translate([ 0, 0, OUTER_BOX_DIMS.z / 4 ]) mirror([ 0, 0, flip ? 1 : 0 ])
    ...
    translate([ -lid_extrusion.x / 2, -lid_extrusion.y / 2, -lid_extrusion.z/2-BOX_THICKNESS*2 ]) #cube(
        [ lid_extrusion.x, lid_extrusion.y, lid_extrusion.z ], center = false);
  }
}

module internal() {
  outer_dims = [
    INNER_BOX_DIMS.x + BOX_THICKNESS, INNER_BOX_DIMS.y + BOX_THICKNESS,
    INNER_BOX_DIMS.z
  ];
...
module thumb_bit(dims = [ 15, 25, 200 ], radius = 4) {
  ...
  translate([0, -dims.z/2, -dims.y/2]) #rotate([ -90, 0, 0 ]) linear_extrude(height = dims.z)
...
difference() {
  ...
  translate([0, 0, INNER_BOX_DIMS.z+BOX_THICKNESS*2]) thumb_bit(dims = [ 20, 25, 200 ], radius = 4);
}